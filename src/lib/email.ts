...
// Generic email send function via Brevo
export async function sendEmail({
  to,
  subject,
  html,
  senderName = "שיעורי מרתון עם רועי",
  senderEmail,
}: {
  to: string;
  subject: string;
  ...
  const fromEmail = senderEmail || process.env.BREVO_SENDER_EMAIL || "noreply@marathon-lessons.com";
  ...
  console.log("Brevo email sent to:", to, "response:", JSON.stringify(response));
  return response;
}

// Lesson email with template support
